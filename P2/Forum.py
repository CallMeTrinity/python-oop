...
modo = Moderator("William", "admin")

thread = user.make_thread("Titre", "HI")
user.post(thread, "Bonjour")
modo.post(thread, "Bonjour toi")
user.post(thread, "Pomme de terre")
modo.post(thread, "hors sujet")
modo.delete(thread, thread.posts[-1])
modo.delete(thread, thread.posts[-1])
image = ImageFile("image.png", 200)
user.post(thread, "Pomme de terre", image)
thread.display()